...
export default class Keji extends Component {
    constructor(props) {
        super(props);
    ...
    componentDidMount() {
        fetch("http://localhost:9000/index",{
            method: 'POST', // or 'PUT'
            body:JSON.stringify({name:encodeURI('科技')}), // data can be `string` or {object}!
            headers: 
            ...
            <div>
                ...
                <div className="content">
                    ...
                    this.state.totaldata.map(function(name){
                        return(
                        <li key={name.id}>
                            <Link to={{pathname:'/shouye/fenlei/keji/xiangqing',state:{name:name.herf}} }>
