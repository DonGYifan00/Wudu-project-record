...
componentDidMount() {
    fetch("http://localhost:9000/index",{
        method: 'POST', // or 'PUT'
        body:JSON.stringify({name:encodeURI('美食')}), // data can be `string` or {object}!
        headers: 
            ...
            <div>
                ...
                <div className="content">
                    ...
                    this.state.totaldata.map(function(name){
                        return(
                        <li key={name.id}>
                            <Link to={{pathname:'/shouye/tuijian/tuimeishi/xiangqing',state:{name:name.herf}} }>
